...
import EmptyInput from "../inputs/empty_input/EmptyInput";
import { useSignalR } from "../SignalR/SignalRContext";
import styles from "./ChatMessages.module.css"
import Message from "../ChatMessage/Message";

const ChatMessages = ({chatId, messages=[]}) => {
  const connection = useSignalR();
  const [currentMessage, setCurrentMessage] = useState("");

  const handleSendMessage = () => {
    connection.handleSendMessage(currentMessage, chatId);
  ...
  return(
    <div className={styles.ChatMessages__Main}>
      <div className={styles.ChatMessages__Messages}>
        {
          messages
          .map(message => (
